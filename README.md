...
parameter. The internal slot is centered on this thickness so it overwrites any
overlapping base material. Its size is slightly larger than the frame opening,
controlled by `photoframe_slot_margin`, allowing a printed photo to slide in
even when the frame merges with the plate.
